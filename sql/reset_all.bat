...
"c:\program files\mysql\mysql server 5.6\bin\mysqladmin"-h localhost -u root -pubsidian DROP dspdb

ECHO Creating Database dspdb
"c:\program files\mysql\mysql server 5.6\bin\mysqladmin"-h localhost -u root -pubsidian CREATE dspdb

ECHO Loading dspdb tables into the database
cd C:\Users\Ryan\Documents\darkstar-df\sql
FOR %%X IN (*.sql) DO ECHO Importing %%X & "c:\program files\mysql\mysql server 5.6\bin\mysql" dspdb -h localhost -u root -pubsidian < %%X
